...
/obj/item/gun
	...
	/// The amount of heat generated by this gun when shot
	var/heat_gen = 55
	/// The amount of heat this gun contains
	var/contained_heat = 0
	/// The max amount of heat this gun could hold
	var/heat_capacity = 200
	/// The rate at which the heat passively cools off
	var/heat_cooldown_rate = 50
	/// The timer for cooldown
	var/cooldown_timer
	/// Overheat locked
	var/overheated = FALSE

/// return false if bad shot
/obj/item/gun/proc/check_cooldown(mob/user)
	if (overheated)
		to_chat(user, span_notice("Your gun cannot fire right now!"))
		return FALSE
	var/threshold = get_heat_threshold()
	if (threshold >= 100)
		playsound(user, 'sound/effects/alert.ogg', 50, TRUE)
		to_chat(user, span_alertwarning("Your gun has become dangerously hot, and overheated!"))
		overheated = TRUE
		return FALSE
	if (threshold >= 80)
		to_chat(user, span_warning("Your gun is becoming very hot!"))
		playsound(user, 'sound/machines/twobeep.ogg', 20, TRUE)
		return TRUE
	if (threshold >= 65)
		playsound(user, 'sound/machines/twobeep.ogg', 20, TRUE)
		to_chat(user, span_notice("Your gun begins to heat up."))
		return TRUE
	return TRUE

/obj/item/gun/proc/heat_cooldown()
	if (QDELETED(src))
		return
	contained_heat = max(0, contained_heat - heat_cooldown_rate)
	var/threshold = get_heat_threshold()
	if (threshold > 0)
		cooldown_timer = addtimer(CALLBACK(src, .proc/heat_cooldown), 1 SECONDS)
	else
		if (overheated)
			visible_message(span_notice("[src] seems to have cooled off."))
		overheated = FALSE


/obj/item/gun/proc/get_heat_threshold()
	return min(100, (contained_heat / heat_capacity) * 100)
...
/obj/item/gun/examine(mob/user)
	...
	if(overheated)
		. += "It is bright red and extremely hot!"
	else
		switch(get_heat_threshold())
			if (1 to 64)
				. += "It is a little warm."
			if (65 to 80)
				. += "It is throwing off heat."
			if (80 to INFINITY)
...
/obj/item/gun/proc/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	contained_heat += heat_gen
	if (contained_heat == heat_gen)
		heat_cooldown()
